...
const TranscribeWords = () => {
	...
	const fetchWords = async () => {
		...
		const word = await fetch(`${api}/transcribe/transcribe_words`, {
			method: 'GET',
		})
			.then((res) => {
					...
					<div className="border-2 border-dashed border-yellow-500 px-3 py-2 rounded-3xl h-full w-full">
						<div className="relative flex flex-col justify-center items-center h-full w-full bg-yellow-100 rounded-xl p-2 ">
							<Nav
								...
								forwardLink="/transcribers/transcribe-sentences"
								color="gray-400"
								hoverColor="yellow"
							/>
							{isLoading ? (
								...
								<div
									...
									className="flex flex-col justify-around text-center h-full py-20"
								>
									<div className="text-3xl md:text-4xl md:text-5xl font-bold text-yellow-500">
										{word}
									</div>
									<div
										className={`${
							...
							<div className="w-full flex flex-col items-center justify-center gap-4">
								<div className="flex justify-around w-full">
									...
									<div>
										<button
											...
											className={`text-4xl ${
												isNext ? 'text-gray-500' : 'animate-pulse text-red-400'
											}`}
								...
								<div className="flex justify-center w-full">
									<button
										...
										className={`py-2 w-full max-w-[200px] md:max-w-[250px] border-2 border-dashed border-${
											isCorrect
